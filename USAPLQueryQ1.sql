...
************/
-- Returns average total for all weight classes based on WeightClassKg (There are some things that are wrong with this)
SELECT
	Sex,
	WeightClassKg,
	Year(Date) as Year,
...
WHERE TotalKg is not null AND (Division = 'MR-O' OR Division = 'FR-O') 
GROUP BY Sex, SortOrder, WeightClass, Year -- Groups by Sex, then SortOrder (int WeightClass lowerbound), then WeightClass (shouldn't matter, just for looks), then year
ORDER BY Sex, SortOrder, Year -- Orders by Sex first, then SortOrder (not shown in table, but sorts by integer), then year
